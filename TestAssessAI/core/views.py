    ...
    for subject in subjects:
        ...
        for test in tests:
            submissions = Submission.objects.filter(
                student=request.user, question__test=test)
            total_score += submissions.aggregate(Sum('mark__teacher_mark'))[
                'mark__teacher_mark__sum'] or 0

        subject_tests.append(
            {'subject': subject, 'tests': tests, 'total_score': total_score})
...
def view_test_student(request, test_id):
    ...
    if request.method == 'POST':
        form_questions = [Question(question_id=int(key.split('question_answer_')[1]), question_anwser=value) for key,
